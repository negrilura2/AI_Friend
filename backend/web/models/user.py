...
def photo_upload_to(instance, filename):
    ext = filename.split('.')[-1] #取文件名后缀，如photo.png,会取得png。
    filename = f'{uuid.uuid4().hex[:10]}.{ext}'# 文件名使用python中的随机数uuid
    return f'user/photos/{instance.user_id}_{filename}'
    #instance是数据库的对象，user_id能比user.id少一次数据库的索引。只要有user,user_id就是user的id。
