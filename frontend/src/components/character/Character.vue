  ...
  <div>
    <div class="avatar cursor-pointer"@mouseover="isHover=true" @mouseout="isHover=false" @click="openChatField">
      <div class="w-60 h-100 rounded-2xl relative">
        <div >
<!--          <img :src="character.photo" class="absolute left-16 top-12 w-28 h-28 flex justify-center items-center bg-black/20 rounded-full transition-transform duration-300 cursor-pointer" @mouseover="isHover=true" @mouseout="isHover=false" :class="{'scale-120':isHover}" alt="">-->
          ...
          <div class="absolute left-4 top-46 avatar">
            <div class="w-16 rounded-full ring-3 ring-white">
              <img :src="character.photo" alt="">
            </div>
          </div>
          <div class="absolute left-24 top-52 text-white font-bold line-clamp-1 break-all">
            {{character.name}}
          </div>
          <div class="absolute left-4 right-4 top-65 text-white line-clamp-5 break-all">
            {{character.profile}}
          </div>
        </div>
      </div>
